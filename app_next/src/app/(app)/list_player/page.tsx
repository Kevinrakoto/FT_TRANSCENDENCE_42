...
type Message = {
  id: string;          // socketId émetteur
  content: string;
  timestamp: number;
  room_name: string;
  user: string;        // userId émetteur
};
...
export default function OnlinePlayersPage() {
  ...
  const [myFriends, setMyFriends] = useState<string[]>([]); // liste d'userId amis
  ...
  // 3. Connect socket and get online players
  useEffect(() => {
    if (!currentUser) return;
    ...
    const onPlayersUpdate = (players: OnlinePlayer[]) => {
      setOnlinePlayers(players.filter(p => p.userId !== currentUser.id)); // exclude self
    };

    const onNewMessage = (msg: Message) => {
      // N'ajouter que les messages de la conversation courante
