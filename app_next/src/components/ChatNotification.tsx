...
export default function ChatNotification() {
  ...
  if (notifications.length === 0 && unreadFriendRequestsCount === 0) return null
  ...
  return (
    <div className="fixed top-4 right-4 z-50">
      <button
        ...
        className="relative bg-gray-800 hover:bg-gray-700 text-white p-3 rounded-full shadow-lg transition-all border border-gray-600"
      >
            ...
            <div className="max-h-80 overflow-y-auto">
              ...
              {unreadFriendRequestsCount > 0 && (
                <div className="p-4 text-center">
                  <span className="text-yellow-500 font-semibold">{unreadFriendRequestsCount}</span>
                  <span className="text-gray-400 ml-2">pending request{unreadFriendRequestsCount > 1 ? 's' : ''}</span>
                </div>
              )}
            </div>
          )}
